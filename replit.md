...
-   **Carton Tracking**: Orders track carton counts as separate small and large fields (`small_cartons`, `large_cartons` integer columns). The backend auto-derives the total `cases` as their sum (for transport cost allocation). A one-time migration (April 2026) moved all legacy `cases` values into `large_cartons` for the 655 orders that had a case count but no small/large breakdown — so all orders now have their carton data in the structured fields. The UI shows "Xs + Yl" format when both fields are set, or just the large-carton count for single-type orders.
