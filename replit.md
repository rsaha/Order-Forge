...
-   **My Orders Page** (`/sales-orders`): Unified order view for all roles powered by `GET /api/portal/orders`. Features: status tabs (color-coded, role-filtered — BrandAdmin hides PODReceived, Customer sees limited tabs), search bar (party, invoice, brand, creator), date presets (Today/7d/30d/This Month/Last Month/All), brand filter (Admin: all brands; BrandAdmin: own brands only), delivery company filter (Admin only). Admin also gets Export (XLSX summary or details) and Bulk WhatsApp (grouped by brand/delivery company for dispatched or delivered orders). Approve button available for non-Customer users on Created orders. Order detail dialog shows item breakdown. API accepts `brand` and `deliveryCompany` query params for filtering at source.
